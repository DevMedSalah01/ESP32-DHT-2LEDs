...
#define DHTPIN 21          //--> Defines the Digital Pin connected to the DHT22 sensor.
...
const char* mqtt_topic = "MESSAGE_RECEIVED";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.println();
  // Vérifier le message reçu et mettre à jour l'état de la LED en conséquence
  if (strcmp(topic, mqtt_topic) == 0) {
    if (payload[0] == '1') {
      digitalWrite(LED_01, HIGH);  // Allumer la LED
    } else {
      digitalWrite(LED_01, LOW);  // Éteindre la LED
    }
  }
}
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    digitalWrite(ON_Board_LED, HIGH);//get request sign
    delay(250);
    digitalWrite(ON_Board_LED, LOW);//update http request sign
    delay(250);
    //........................................
