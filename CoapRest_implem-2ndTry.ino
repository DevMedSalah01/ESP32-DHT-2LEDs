...
#define DHTPIN 21 //--> Defines the Digital Pin connected to the DHT11 sensor.
...
void loop() {
  ...
  if(WiFi.status()== WL_CONNECTED) {
    ...
    digitalWrite(ON_Board_LED, LOW);
    //........................................ 
      // Control LED_01 via CoAP
  control_LED_via_CoAP(LED_01, 1); // Turn on LED_01
  delay(2000);
  control_LED_via_CoAP(LED_01, 0); // Turn off LED_01
  delay(2000);

  // Control LED_02 via CoAP
  control_LED_via_CoAP(LED_02, 1); // Turn on LED_02
  delay(2000);
  control_LED_via_CoAP(LED_02, 0); // Turn off LED_02
  delay(2000);
    delay(4000);
  }
  //---------------------------------------- 
...
void control_LED_via_CoAP(int led, int state) {
  if (WiFi.status() == WL_CONNECTED) {
    CoAP coapClient;
    coapClient.begin("coap://192.168.1.5/led");

    if (state == 0) {
      coapClient.set(CoAP::PUT, "0");
    } else {
      coapClient.set(CoAP::PUT, "1");
    }

    int result = coapClient.send();
    if (result == COAP_RESULT_OK) {
      String response = coapClient.lastResponse();
      Serial.println(response);
    } else {
      Serial.println("Error calling CoAP server");
    }
